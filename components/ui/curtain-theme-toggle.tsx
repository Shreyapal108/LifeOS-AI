...
// ─── Icons (Replacing with Lucide) ────────────────────────────────────────────

function MoonIcon() {
  return <Moon size={18} strokeWidth={2.5} />;
}

function SunIcon() {
  return <Sun size={18} strokeWidth={2.5} />;
}
...
export function ThemeToggle({
  ...
  const pageStyle: CSSProperties = {
    minHeight: "100vh",
    paddingTop: variant === "icon" ? 0 : barHeight,
    background: variant === "icon" ? "transparent" : t.pageBg,
    color: t.pageText,
